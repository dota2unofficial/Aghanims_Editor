<template>
	<v-app-bar app color="primary" dark>
		<v-select
			...
			class="mr-5"
		></v-select>
		<v-btn dark @click="saveConfirm" outlined class="mr-3">
			Save
		</v-btn>

		<v-btn dark @click="showDebugger" outlined>
			Show Debugger
		</v-btn>

		<v-dialog v-model="confirmSave" persistent>
			<v-card>
				<v-card-title>Confirm Save</v-card-title>
				<v-card-text>Are you really sure to save it?</v-card-text>
				<v-card-actions>
					<v-spacer></v-spacer>
					<v-btn color="primary" outlined @click="confirmSave = false"
						>Cancel</v-btn
					>
					<v-btn
						color="primary"
...
export default {
	...
	watch: {
		getD2Found(found) {
			...
			try {
				const files = readdirSync(`${d2path}\\dota_addons\\`, {
					withFileTypes: true
				})
				...
				this.addonList = files.map(file => {
					...
					const root = vdfextra.parse(result, {
						parseUnquotedStrings: true
					});
					return {
						value: file,
						text: root.Tokens["addon_game_name"]
							? root.Tokens["addon_game_name"]
							: file
					};
				});
			} catch (err) {
